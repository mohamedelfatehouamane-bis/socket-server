...
app.post('/api/internal/topup-status', (req, res) => {
  const secret = req.headers['x-internal-secret']
  const expectedSecret = process.env.SOCKET_INTERNAL_EVENT_SECRET

  if (!expectedSecret || secret !== expectedSecret) {
    console.warn('[SocketServer][Internal] Unauthorized attempt to emit topup status update')
    return res.status(401).json({ error: 'Unauthorized' })
  }

  const { userId, topupId, status, amountPoints } = req.body
  if (!userId || !topupId || !status) {
    return res.status(400).json({ error: 'Missing required fields' })
  }

  console.log(`[SocketServer][Internal] Emitting topup_status_update for user ${userId}, topup ${topupId}, status ${status}`)

  const payload = {
    userId,
    topupId,
    status,
    amountPoints: amountPoints ? Number(amountPoints) : undefined,
    timestamp: new Date().toISOString(),
  }

  io.to(`user_${userId}`).emit('topup_status_update', payload)
  io.to('admin').emit('topup_status_update', payload)
